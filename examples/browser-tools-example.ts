...
async function browserToolsExample() {
  ...
  const demoTask =
    'Visit https://example.com and describe what you see on the page';
...
async function directBrowserExample() {
  ...
  const { VisitPageTool, PageDownTool, FindOnPageTool } = await import(
  ...
  try {
    ...
    console.error(
      'Error in direct browser example:',
      error instanceof Error ? error.message : String(error)
    );
  }
}
