...
async function main() {
  ...
  try {
    ...
    const mathResult = await agent.execute(
      'What is 15 + 27? Use the add_numbers tool and provide the final answer.'
    );
    console.log(
      'Math Answer:',
      mathResult.data?.answer || mathResult.data || 'No response received'
    );
    ...
    console.log(
      'UUID Answer:',
      uuidResult.data?.answer || uuidResult.data || 'No response received'
    );
  } catch (error) {
