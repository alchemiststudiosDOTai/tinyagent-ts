...
describe('Examples E2E Smoke Tests', () => {
  ...
  const shouldSkip = !process.env.OPENROUTER_API_KEY && !process.env.SMOLAGENTS_E2E;

  beforeAll(() => {
    if (shouldSkip) {
      console.log('Skipping E2E tests: No OPENROUTER_API_KEY or SMOLAGENTS_E2E environment variable');
    }
  });
  ...
  examples.forEach(exampleFile => {
    it(`should run ${exampleFile} without returning undefined`, async () => {
      if (shouldSkip) {
