import { VisitPageTool, PageDownTool, FindOnPageTool } from '../src/tools/browser-tools';
...
async function testBrowserDirectly() {
  ...
  try {
    ...
    const visitResult = await visitTool.execute({ url: 'https://tinyagent.xyz/' });
    
    console.log('📄 Visit Result:');
    console.log('-'.repeat(50));
    console.log(visitResult.slice(0, 1000) + (visitResult.length > 1000 ? '\n... (truncated)' : ''));
    console.log('-'.repeat(50));
    ...
    console.log('-'.repeat(50));
    console.log(scrollResult.slice(0, 1000) + (scrollResult.length > 1000 ? '\n... (truncated)' : ''));
    console.log('-'.repeat(50));
    ...
    console.log('-'.repeat(50));
    console.log(searchResult.slice(0, 1000) + (searchResult.length > 1000 ? '\n... (truncated)' : ''));
    console.log('-'.repeat(50));
