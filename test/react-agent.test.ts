...
describe('ReAct Agent Test', () => {
  ...
  beforeEach(() => {
    ...
    const tools = getDefaultTools().filter(
      (tool) => tool.name !== 'duckSearch' && tool.name !== 'final_answer'
    );
  ...
  it('should use tools and provide final answer', async () => {
    const result = await agent.execute(
      'Generate a UUID and tell me what it is'
    );
    ...
    steps.forEach((step: any, index: number) => {
      if (step.type === 'thought') {
        ...
        console.log(
          `Step ${index + 1} - Action: ${step.tool}(${JSON.stringify(step.args)})`
        );
