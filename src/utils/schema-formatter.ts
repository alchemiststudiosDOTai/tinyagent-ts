 ...
 */
function formatZodTypeForPrompt(key: string, zodType: z.ZodTypeAny): string | null {
  try {
 ...
 */
export function extractZodSchemaExamples(schema: z.ZodSchema<any>): Record<string, any> {
  try {
    if (schema instanceof z.ZodObject) {
      const shape = schema.shape as Record<string, z.ZodTypeAny>;
  ...
  try {
    ...
    switch (typeName) {
      ...
      case 'ZodBoolean':
        return true;
      case 'ZodArray':
        const elementExample = getZodTypeExample((innerType as z.ZodArray<any>).element);
