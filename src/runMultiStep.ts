...
export async function runMultiStep<I = string>(
  ...
  const toolCatalog = Object.values(tools)
    ...
    .join('\n');
  const system = (agent as any).promptEngine.render('agent', {
    tools: toolCatalog,
  });
  ...
  for (let step = 0; step < maxSteps; step++) {
    const resp = await (agent as any).makeOpenRouterRequest(
      messages,
      modelName
    );
    ...
    messages.push({
      role: 'assistant',
      content: JSON.stringify({ observation: String(result) }),
    });
  }
